#!/usr/bin/env python3
"""
Train a contrastive question-to-chunk retriever using static layer pooling.
...
class QuestionChunkDataset(Dataset):
    def __init__(
        ...
        for qa in qa_pairs:
            gold_doc_ids = extract_gold_doc_ids(qa)
            question = qa.get("question", "").strip()
            if not gold_doc_ids or not question:
                continue
            chunk_idx = None
            for doc_id in gold_doc_ids:
                idx = chunk_index.get(doc_id)
                if idx is not None:
                    chunk_idx = idx
                    break
...
class StaticLayerWeights(nn.Module):
    """
    Learnable global layer weights for pooling across KV layers.
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--log-every", type=int, default=200, help="Logging frequency (steps).")
...
def save_heads_checkpoint(
    ...
    static_pool: StaticLayerWeights,
    output: Dict,
) -> None:
    ...
    payload = {
        ...
        "static_pool": static_pool.state_dict(),
        "meta": {
...
def load_heads_checkpoint(
    ...
    static_pool.load_state_dict(payload["static_pool"])
...
def train_contrastive(
    ...
    val_pairs: List[Dict],
    args: argparse.Namespace,
) -> Tuple[ProjectionHead, ProjectionHead, StaticLayerWeights, torch.Tensor]:
    ...
    question_head = ProjectionHead(base_model.config.hidden_size, args.proj_dim).to(device)
    chunk_head = ProjectionHead(chunk_dim, args.proj_dim).to(device)
    static_pool = StaticLayerWeights(chunk_layer_matrix.shape[0]).to(device)
    opt = torch.optim.Adam(
        list(question_head.parameters()) + list(chunk_head.parameters()) + list(static_pool.parameters()),
        lr=args.lr,
    )

    global_step = 0
    best_val = -1.0
    best_q_state, best_c_state = None, None
    ...
    for epoch in range(args.epochs):
        for batch_questions, chunk_idx in loader:
            ...
            opt.zero_grad()
            q_emb = embed_questions(batch_questions, base_model, device).to(device).float()
            q_proj = question_head(q_emb)

            # Static layer pooling shared across questions.
            pool_w = static_pool()  # (layers,)
            chunk_vec = torch.einsum("l,lnd->nd", pool_w, chunk_layer_matrix)
            chunk_proj = chunk_head(chunk_vec)  # (num_chunks, proj_dim)
            logits = torch.matmul(q_proj, chunk_proj.t()) / args.temperature
    ...
    if best_q_state is not None and best_c_state is not None:
        question_head.load_state_dict(best_q_state)
        chunk_head.load_state_dict(best_c_state)

    return question_head, chunk_head, static_pool, chunk_layer_matrix
...
def evaluate(
    ...
    for idx, qa in enumerate(qa_pairs):
        ...
        with torch.no_grad():
            pool_w = static_pool()  # (layers,)
            chunk_vec = torch.einsum("l,lnd->nd", pool_w, chunk_layer_matrix)
            chunk_proj = chunk_head(chunk_vec)
            q_proj = question_head(q_vec)
            scores = torch.matmul(q_proj, chunk_proj.t()).squeeze(0) / args.temperature
        top_idx = int(torch.argmax(scores).item())
        pred_doc = doc_ids[top_idx]
        is_correct = pred_doc in gold_doc_ids
        correct += int(is_correct)

        gold_indices = [doc_to_idx[doc_id] for doc_id in gold_doc_ids if doc_id in doc_to_idx]
        if gold_indices:
            for k in k_list:
                topk_idx = torch.topk(scores, k=k).indices
                if any((topk_idx == gold_idx).any() for gold_idx in gold_indices):
                    recall_counts[k] += 1
            sorted_idx = torch.argsort(scores, descending=True)
            ranks = []
            for gold_idx in gold_indices:
                pos = (sorted_idx == gold_idx).nonzero(as_tuple=True)
                if len(pos[0]) > 0:
        ...
        predictions.append(
            {
                "qid": qa.get("qid"),
                "question": question,
                "gold_doc_id": gold_doc_ids[0] if gold_doc_ids else None,
    ...
    metrics["hit_rate"] = recall_counts.get(10, 0) / total
    metrics["mrr"] = mrr_sum / total
    for k in k_list:
        rec = recall_counts[k] / total
        metrics[f"recall@{k}"] = rec
        metrics[f"precision@{k}"] = rec / k
    print(
        "  Hit Rate       : {0:.3f}\n"
        "  MRR            : {1:.3f}\n"
        "  Recall@1       : {2:.3f}    Precision@1 : {3:.3f}\n"
        "  Recall@3       : {4:.3f}    Precision@3 : {5:.3f}\n"
        "  Recall@5       : {6:.3f}    Precision@5 : {7:.3f}\n"
        "  Recall@10      : {8:.3f}    Precision@10: {9:.3f}".format(
            metrics["hit_rate"],
            metrics["mrr"],
            metrics["recall@1"],
            metrics["precision@1"],
...
def main():
    ...
    if args.eval_only:
        ...
        question_head = ProjectionHead(base_model.config.hidden_size, args.proj_dim).to(primary_device)
        chunk_head = ProjectionHead(chunk_layer_matrix.shape[2], args.proj_dim).to(primary_device)
        static_pool = StaticLayerWeights(chunk_layer_matrix.shape[0]).to(primary_device)
        meta = load_heads_checkpoint(args.load_heads, question_head, chunk_head, static_pool)
    ...
    question_head, chunk_head, static_pool, chunk_layer_matrix = train_contrastive(
        train_dataset, chunk_entries, base_model, tokenizer, primary_device, val_pairs, args
    )
    ...
    print(f"Wrote predictions to {args.output_json}")
    if args.save_heads:
        save_heads_checkpoint(args.save_heads, question_head, chunk_head, static_pool, output)
