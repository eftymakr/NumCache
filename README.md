...
├── contrastive/
│   ├── mlp_retrieve_topk.py                  # Contrastive retriever (Sec. 3.2) — train + score
...
│   ├── baseline_with_correct_cache.py        # NumCache gold-cache inference
...
| Sec. 3.2 — contrastive retrieval (dual heads, query-adaptive pooling) | `contrastive/mlp_retrieve_topk.py`, `contrastive/eval_contrastive_recall.py` |
...
| Table 3 — Full Context baseline (Qwen3-4B / GPT-5.1) | `inference/baseline_fullcontext.py`, `inference/gen_gpt51.py` |
...
The first-p initialization baseline reported in Tables 6 and 7 is produced by `automated_training_pipeline.py --init-method pinit`.
...
# CUDA-capable GPU for cache training, contrastive retrieval, and vLLM
```
...
### 4. Contrastive KV retriever + cache-concat inference

```bash
# Evaluate retriever recall (Table 2)
python contrastive/eval_contrastive_recall.py

# Generate answers using top-K cache retrieval + concatenation
...
- Trained cache files (`cache_last.pt`) and projection heads (`projection_heads.pt`) are produced by the training pipeline and are **not** included in this repo.
- A few scripts contain hardcoded `BASE_DIR` paths from the original development environment — update them to your paths before running.
- API keys are not committed. Set `OPENAI_API_KEY` (GPT-5.1) and `AZURE_OPENAI_API_KEY` (GPT-4.1 judge) via environment variables.
